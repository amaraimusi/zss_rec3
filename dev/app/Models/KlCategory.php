...
class KlCategory extends CrudBase
{
	protected $table = 'kl_categorys'; // 紐づけるテーブル名
	...
	public function getFieldData(){
		...
		$fieldData = $this->addFieldDetailsFromDB($fieldData, 'kl_categorys');
	...
	public function getData($searches, $param=[]){
		...
		$query = DB::table('kl_categorys')->
			leftJoin('users', 'kl_categorys.update_user_id', '=', 'users.id');
		
		$query = $query->select(
				'kl_categorys.id as id',
				// CBBXS-6019
				'kl_categorys.id as id',
				'kl_categorys.kl_category_name as kl_category_name',
				'kl_categorys.category_code as category_code',

				// CBBXE
				'kl_categorys.sort_no as sort_no',
				'kl_categorys.delete_flg as delete_flg',
				'kl_categorys.update_user_id as update_user_id',
				'users.nickname as update_user',
				'kl_categorys.ip_addr as ip_addr',
				'kl_categorys.created_at as created_at',
				'kl_categorys.updated_at as updated_at',
		...
		if(!empty($searches['main_search'])){
			$concat = DB::raw("
					CONCAT( 
					/* CBBXS-6017 */
					IFNULL(kl_categorys.kl_category_name, '') , 
					IFNULL(kl_categorys.category_code, '') , 
					IFNULL(kl_categorys.update_user, '') , 
					IFNULL(kl_categorys.ip_addr, '') , 
	...
	private function addWheres($query, $searches){
		...
		if(!empty($searches['id'])){
			$query = $query->where('kl_categorys.id',$searches['id']);
		}
		...
		// カテゴリ名
		if(!empty($searches['kl_category_name'])){
			$query = $query->where('kl_categorys.kl_category_name', 'LIKE', "%{$searches['kl_category_name']}%");
		}

		// カテゴリコード
		if(!empty($searches['category_code'])){
			$query = $query->where('kl_categorys.category_code', 'LIKE', "%{$searches['category_code']}%");
		}
		...
		if(!empty($searches['delete_flg']) || $searches['delete_flg'] ==='0' || $searches['delete_flg'] ===0){
			if($searches['delete_flg'] != -1){
				$query = $query->where('kl_categorys.delete_flg',$searches['delete_flg']);
			}
		}
		...
		if(!empty($searches['update_user'])){
			$query = $query->where('kl_categorys.update_user', 'LIKE', "%{$searches['update_user']}%");
		}

		// IPアドレス
		if(!empty($searches['ip_addr'])){
			$query = $query->where('kl_categorys.ip_addr', 'LIKE', "%{$searches['ip_addr']}%");
		}

		// 生成日時
		if(!empty($searches['created_at'])){
			$query = $query->where('kl_categorys.created_at', '>=', $searches['created_at']);
		}

		// 更新日時
		if(!empty($searches['updated_at'])){
			$query = $query->where('kl_categorys.updated_at', '>=', $searches['updated_at']);
		}
		...
		// 順番
		if(!empty($searches['sort_no'])){
			$query = $query->where('kl_categorys.sort_no',$searches['sort_no']);
		}
		...
		if(!empty($searches['delete_flg'])){
			...
			$query = $query->where('kl_categorys.delete_flg', 0);
		}
	...
	public function nextSortNo(){
		$query = DB::table('kl_categorys')->selectRaw('MAX(sort_no) AS max_sort_no');
		$res = $query->first();
