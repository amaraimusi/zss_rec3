...
<head>
	...
	<script src="{{ asset('/js/Diary/create.js')  . $ver_str}} }}" defer></script>
	...
	<link href="{{ asset('/css/Diary/create.css')  . $ver_str}}" rel="stylesheet">
	
	<title>日誌管理・新規登録フォーム</title>
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
	...
	<li class="breadcrumb-item active" aria-current="page">日誌管理・新規登録フォーム</li>
  </ol>
</nav>
...
<div>
	<div class="form_w" >
		<form id="form1" method="POST" action="{{ url('diary/store') }}" onsubmit="return checkDoublePress()" enctype="multipart/form-data">
			@csrf
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="category" type="date"  class="form-control form-control-lg" placeholder="category" value="{{old('category', $ent->category)}}">
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="diary_date" type="text"  class="form-control form-control-lg" placeholder="diary_date" value="{{old('diary_date', $ent->diary_date)}}" pattern="[0-9]{4}(-|/)[0-9]{1,2}(-|/)[0-9]{1,2} [0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2}" title="日時（Y-m-d H:i:s)を入力してください。(例  2012-12-12 12:12:12)">
				</div>
			</div>
