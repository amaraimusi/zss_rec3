...
$(()=>{
	...
	crudBase = new CrudBase4(crudBaseData,{
		...
		'create_tr_place': 1, // 新規登録後の行の挿入場所： 0:末尾 , 1:先頭
	});
	...
	rowExchange = new RowExchange('main_tbl', data, null, (param)=>{
		...
		let auto_save_url = 'diary/auto_save';
...
function disabledBtn(btnElm, action_flg){
	...
	let url = 'diary/disabled'; // Ajax通信先URL
...
function destroyBtn(btnElm){
	...
	let json_str = JSON.stringify(data);//データをJSON文字列にする。
	let url = 'diary/destroy'; // Ajax通信先URL
...
function regAction(){
	...
	// SPA型・登録アクション
	crudBase.regAction(ent,'diary/reg_action', {
		'callback': (param)=>{
			// DB登録後のコールバック処理内容
