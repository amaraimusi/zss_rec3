...
$(()=>{
	...
	rowExchange = new RowExchange('main_tbl', data, null, (param)=>{
		...
		let auto_save_url = 'kl_category/auto_save';
...
function disabledBtn(btnElm, action_flg){
	...
	let url = 'kl_category/disabled'; // Ajax通信先URL
...
function destroyBtn(btnElm){
	...
	let json_str = JSON.stringify(data);//データをJSON文字列にする。
	let url = 'kl_category/destroy'; // Ajax通信先URL
...
function regAction(){
	...
	// SPA型・登録アクション
	crudBase.regAction(ent,'kl_category/reg_action', {
		'callback': (param)=>{
			// DB登録後のコールバック処理内容
...
function clearA(){
	...
	location.href = 'kl_category?clear=1';
}
